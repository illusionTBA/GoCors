...
func main() {
	portPtr := flag.Int("p", 3000, "set port")
	flag.Parse()
	fmt.Println(*portPtr)
	app := fiber.New()
	...
	app.Get("/", func(c *fiber.Ctx) error {
		return c.JSON(fiber.Map{
			"message": "Welcome to GoCors more info at https://github.com/illusionTBA/GoCors",
			"Routes": fiber.Map{
				"/cors": fiber.Map{
					"message":      "where you will be requesting the most",
					"query-params": "?url (required) & headers (optional)",
					"example":      "?ur=https://example.com&headers=Some-Header:Some-Value,Another-Header:Another-Value",
				},
			},
		})
	})

	app.All("/cors", func(c *fiber.Ctx) error {
		return Proxy(c)
	})

	log.Fatal(app.Listen(fmt.Sprintf(":%d", *portPtr)))
}
